...
Kennedy from [O'Reilly] (2nd. Ed.) as the definative reference.
...
# modifications by JSB on lines 1150, 1152, 2151, 2168 to handle double
# quotes appearing in Textarea and Input (type text) objects
# 7/8/98
...
class Area:
    ...
    def __str__(self):
        ...
        if self.href: s.append(' HREF="%s"' % self.href)
        else: s.append(' NOHREF')
        s.append('>\n')
        return string.join(s, '')
...
class Frameset:
    ...
    def __str__(self):
        s = ['\n<FRAMESET']
        if self.rows: s.append(' ROWS="%s"' % self.rows)
        if self.cols: s.append(' COLS="%s"' % self.cols)
        if self.onLoad: s.append(' onLoad="%s"' % self.onLoad)
        if self.onUnload: s.append(' onUnload="%s"' % self.onUnload)
        s.append('>\n')
        if self.frame_warning:
            ...
            <p>
            This document is designed to be viewed using Netscape's
            ...
            <a href="http://home.netscape.com/">Netscape Communications</a> or
...
class Frame:
    ...
    def __str__(self):
        ...
        if self.name: s.append(' NAME="%s"' % self.name)
        if self.marginwidth: s.append(' MARGINWIDTH=%s' % self.marginwidth)
        if self.marginheight: s.append(' MARGINHEIGHT=%s' % self.marginheight)
        if self.scrolling: s.append(' SCROLLING="%s"' % self.scrolling)
        if self.noresize: s.append(' NORESIZE')
        s.append('>\n')
        return string.join(s, '')
...
class Input:
    ...
    def __str__(self):
        ...
        if self.name: s.append('NAME="%s" ' % self.name)
#       if self.value or self.value == 0: s.append('VALUE="%s" ' % self.value)
        if self.value or self.value == 0: s.append('VALUE="%s" ' % \
		escape (str (self.value)))				# JSB
...
class Table:
    ...
    Keyword Parameters
        ...
        heading_color -- a list of color for each column heading (None)
             ...
             the color cycles through the colums
            ...
            if type(self.heading[0]) == ListType:
                for i in range(len(self.heading[0])):
                    middle = middle + '<TH ColSpan=%s>' % \
                             self.colspan[i] + str(self.heading[0][i]) +'</TH>'
                ...
                for i in range(len(self.heading)):
                    middle = middle + '<TH ColSpan=%s>' % \
                             self.colspan[i] + str(self.heading[i]) +'</TH>'
                s.append(prefix + middle + postfix)
...
class Image(AbstractTagSingle):
    ...
    """
    tagname = 'IMG'
    attrs = ('src', 'height', 'width', 'alt', 'border', 'align', 'class',
    ...
    def process_arg(self, arg):
        ...
        if not self.attr_dict['width']: # assume if the user has set the width property
            ...
            try:
                self.width, self.height = getgifsize(self.filename)
...
class HR(AbstractTagSingle):
    ...
    """
    tagname = 'HR'
    attrs = ('align', 'class', 'color', 'noshade', 'size',
             'style', 'width')
    attr_template , attr_dict = _make_attr_inits(attrs)
...
class Heading(AbstractTag):
    ...
    attr_template , attr_dict = _make_attr_inits(attrs)
    def __str__(self):
        if type(self.contents[0]) is not IntType or self.contents[0] not in (1,2,3,4,5,6):
...
class Caption(AbstractTag):
    ...
    """
    tagname = 'CAPTION'
    attrs = ('class', 'style', 'align', 'valign')
    attr_template , attr_dict = _make_attr_inits(attrs)
...
class TH(AbstractTag):
    ...
    """
    tagname = 'TH'
    attrs = ('class', 'style', 'nowrap', 'align','valign','rowspan',
...
class TR(AbstractTag):
    ...
    """
    tagname = 'TR'
    attrs = ('class', 'style', 'align', 'bgcolor', 'bordercolor',
...
class TD(AbstractTag):
    ...
    """
    tagname = 'TD'
    attrs = ('class', 'style', 'nowrap', 'align','valign', 'background',
             'bordercolor', 'bordercolordark', 'bordercolorlight',
...
class TableLite(AbstractTag):
    ...
    """
    tagname = 'TABLE'
    attrs = ('class', 'style', 'align', 'background', 'border',
             'bordercolor', 'bordercolordark', 'bordercolorlight',
...
class Div(AbstractTag):
    ...
    """
    tagname = 'DIV'
    attrs = ('class', 'style', 'align', 'lang', 'nowrap')
    attr_template , attr_dict = _make_attr_inits(attrs)
...
try:
    ...
    def escape(text):
        ...
        text = join(split(text, '>'), '&gt;')
        text = join(split(text, '"'), '&quot;')			# added JSB
        return text
